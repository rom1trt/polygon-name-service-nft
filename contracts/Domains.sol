...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld)
        payable
        ERC721("Rabbit Name Service", "RNS")
    {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Rabbit Name Service", "image": "data:image/svg+xml;base64,',
    ...
    function setRecord(string calldata name, string calldata mail) public {
        // Check that the owner is the transaction sender
        if (msg.sender != domains[name]) revert InvalidName(name);
        records[name] = mail;
        records[name] = mail;
    }
